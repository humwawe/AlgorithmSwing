...
public class MineSweeperData {
    ...
    public static final String blockImageURL = AlgoVisHelper.getResources("block.png");
    public static final String flagImageURL = AlgoVisHelper.getResources("flag.png");
    public static final String mineImageURL = AlgoVisHelper.getResources("mine.png");

    public static String numberImageURL(int num) {
        if (num < 0 || num >= 8) {
            throw new IllegalArgumentException("No such a number image!");
        }
        return AlgoVisHelper.getResources(num + ".png");
    }
    ...
    private void generateMines2(int mineNumber) {
        for (int i = 0; i < mineNumber; i++) {
            int x = i / M;
            int y = i % M;
            mines[x][y] = true;
